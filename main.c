...
void symbol_forget (uclptr_t *symlist, uclptr_t symbol) /* забываем символ (удаляем его из списка) */
{
	...
	if (compare_atoms (symbol, _CAR(_CAR(*symlist)))) /* если символ вдруг находится в голове списка */
	{
		uintptr_t n = _CDR(*symlist); /* нужен следующий элемент */
		list_delete (_CAR(*symlist)); /* удаляем эту запись */
		*symlist = n; /* корректируем голову списка */
	}
	else for (i = *symlist; !IS_NIL(i); i = _CDR(i))
	{
		if (!IS_NIL(_CDR(i))) /* хвост не пуст */
		{
			uclptr_t candidate = _CAR(_CAR(_CDR(i))); /* кандидат - возможно, следующая запись? берём её начало */
			printf ("candidate: "); print_this(candidate); printf ("\n");
			if (compare_atoms (symbol, candidate)) /* сравниваем */
			{
				/* в случае совпадения: */
				uclptr_t n = IS_NIL(_CDR(i)) ? NIL : _CDR(_CDR(i)); /* берём индекс следующей после следующей записи */
				delete_this (_CAR(_CDR(i))); /* удаляем найденную запись */
				_CDR(i) = n; /* корректируем индекс текущей записи */
				break; /* ломаем цикл */
			}
		}
	}
...
/* наивно полагаем, что нам скормили правильные параметры */
{
	...
	{
		uclptr_t candidate = _CAR(_CAR(symrec)); /* CAR(symrec) - первый элемент списка symrec, CAR(CAR)) это первый элемент первого элемента, очевидно */
...
uclptr_t symbol_get_this_value (uclptr_t symrec)
{
	...
	return _CDR(symrec); /* возвращаем список свойств (точнее, единственное свойство - значение) */
}
